...
// Resolve the complete CSS family stack in one FontResolver call. Keeping the
// stack intact is important for platform aliases: CoreText reports `.SF NS`
// for `-apple-system`, which is a successful alias match even though those two
// strings do not overlap.
static sk_sp<SkTypeface> get_cached_typeface(const std::string& family,
                                             int weight, int slant, float size) {
    const auto families = parse_css_font_family_list(family);
#if defined(__ANDROID__)
    if (weight == 400 && slant == 0 && families.size() <= 1 &&
...
static sk_sp<SkTypeface> get_cached_typeface(const std::string& family) {
    return get_cached_typeface(family, SkFontStyle::kNormal_Weight, 0, 14.0f);
}
...
SkFont make_font(const std::string& family, float size,
    ...
    auto typeface = get_cached_typeface(family, weight, slant, size);
    if (!typeface) {
