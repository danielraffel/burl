...
describe('per-corner zero radius identity', () => {
    it('preserves bottom corner zeros through native style', () => {
        const observed: ObservedDomNode = { sourceId: 'corner', tagName: 'div',
            computedStyle: { display: 'block', borderTopLeftRadius: '0px', borderTopRightRadius: '0px', borderBottomLeftRadius: '0px', borderBottomRightRadius: '0px' },
        ...
        expect(typed.paint?.borderTopLeftRadius).toBe(0);
        expect(typed.paint?.borderTopRightRadius).toBe(0);
        ...
        expect(native.root.style?.borderTopLeftRadius).toBe(0);
        expect(native.root.style?.borderTopRightRadius).toBe(0);
    });
});
